...
from trip_planner.serper_lookup import SERPER_SEARCH_URL, lookup_venue
...
@patch("trip_planner.serper_lookup.requests.post")
...
@patch.dict("os.environ", {"SERPER_API_KEY": "test-key"})
@patch("trip_planner.serper_lookup.requests.head")
@patch("trip_planner.serper_lookup.requests.post")
def test_lookup_venue_picks_first_reachable_result(mock_post, mock_head):
...
@patch("trip_planner.serper_lookup.requests.head")
@patch("trip_planner.serper_lookup.requests.post")
def test_lookup_venue_skips_unreachable_result_for_later_reachable_one(mock_post, mock_head):
    mock_post.return_value = _mock_search_response(
        {
            "organic": [
...
@patch("trip_planner.serper_lookup.requests.head")
@patch("trip_planner.serper_lookup.requests.post")
def test_lookup_venue_returns_no_url_when_nothing_reachable(mock_post, mock_head):
    mock_post.return_value = _mock_search_response(
        {
            "organic": [
...
@patch("trip_planner.serper_lookup.requests.head")
@patch("trip_planner.serper_lookup.requests.post")
def test_lookup_venue_treats_head_request_exceptions_as_unreachable(mock_post, mock_head):
    mock_post.return_value = _mock_search_response(
...
@patch("trip_planner.serper_lookup.requests.head")
@patch("trip_planner.serper_lookup.requests.post")
def test_lookup_venue_handles_zero_results(mock_post, mock_head):
    mock_post.return_value = _mock_search_response({"organic": []})
...
@patch("trip_planner.serper_lookup.requests.head")
@patch("trip_planner.serper_lookup.requests.post")
def test_lookup_venue_skips_results_missing_link_when_checking_reachability(mock_post, mock_head):
    mock_post.return_value = _mock_search_response(
        {
            "organic": [
