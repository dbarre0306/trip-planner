...
def _format_context(name: str, interest: str | None, destination: str | None) -> str:
    lines = [f"Venue name: {name}"]
    if interest:
        lines.append(f"Interest category: {interest}")
    if destination:
...
def _build_notes_prompt(
    name: str, interest: str | None, destination: str | None, notes: list[str]
) -> str:
    notes_text = "\n".join(f"- {note}" for note in notes)
    context = _format_context(name, interest, destination)
    return (
...
def _build_fallback_prompt(name: str, interest: str | None, destination: str | None) -> str:
    context = _format_context(name, interest, destination)
    return (
...
def generate_description(
    ...
    if notes:
        prompt = _build_notes_prompt(name, interest, destination, notes)
    else:
        prompt = _build_fallback_prompt(name, interest, destination)
